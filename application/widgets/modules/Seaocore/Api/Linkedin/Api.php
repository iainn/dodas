...
public function parseUserContacts ($contactInfo, $moduletype = null, $Subject = null) {
  ...
  $inviteTableName = $inviteTable->info('name');
	...
	foreach ($contactInfo as $userid => $name) {
		...
		->limit(1)
		->where($inviteTableName . '.social_profileid = ?', $name['id']);
    ...
    if (empty($user_id)) {
		...
		if (!empty($is_site_members->user_id) && $is_site_members->user_id != $user_id) {
			...
			if(!$viewer->isBlocked($contact)) { 
			  ...
			  if (!empty($moduletype) && ($moduletype == 'sitepage' || $moduletype == 'sitebusiness') ) {
...
public function sendInvite ($friendsToJoin, $user_data = null, $pageinvite_id = null, $moduletype = null) {  
  $viewer = Engine_Api::_()->user()->getViewer();
  ...
  if ($moduletype == 'sitepage' || $moduletype == 'sitebusiness') {
   ...
   // $inviteOnlySetting = $settings->getSetting('user.signup.inviteonly', 0);
    if (is_array($friendsToJoin) && !empty($friendsToJoin) && !empty($pageinvite_id) ) {
      ...
      Seaocore_Api_Linkedin_Api::parseUserContacts ($friendsToJoin, $moduletype, $sitepage);
      
      foreach ($friendsToJoin as $recipient) { 
        ...
        if (!empty($recipient)) {
          ...
          if (empty($is_site_members[0]->user_id)) { 
            ...
            $recipient_join[] = $recipient;
            $response_linkedin =  $OBJ_linkedin->message($recipient_join, $response['subject'], $response['body'], FALSE);
      ...
      $response_linkedin =  $OBJ_linkedin->message($friendsToJoin, $response['subject'], $response['body'], FALSE);
      if($response_linkedin['success'] === TRUE) {
        // message has been sent
        //saving the users info in database.
...
public function getBodyText ($mailTemplateType, $mailParams = array()) {
