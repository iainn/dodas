...
function show_contacts_Facebook (id) { 
  $('id_nonsite_success_mess').style.display = 'none';
	...
	$('show_contacts').style.display = 'none';
	if (id == 1) {
		var child_window = window.open (en4.core.baseUrl + 'seaocore/usercontacts/getfacebookcontacts?redirect_uri=' + invite_callbackURl ,'mywindow','width=800,height=500');
	}
	if (window.opener!= null) {
		if (id == 0) {
			var href = window.location.href;
	...
	en4.core.request.send(new Request.JSON( {
	    ...
	    if ($type(responseJSON) && typeof responseJSON.message != 'undefined' && responseJSON.message != '') {
  	    FB.ui({ method: 'apprequests', 
           ...
           exclude_ids: responseJSON.friends_invite
...
function fbInviteRequestCallBack (response) {
       ...
       if ($type (response) && typeof response.error_msg != 'undefined') {
         return;
       }
      
       if ($type (response) && typeof response != 'undefined') {
...
function invitePreview (servicetype) {
	...
	if (checked) {
    url = en4.core.baseUrl + 'sitepageinvite/index/invitepreview?invitepage_id=' + invitepage_id + '&servicetype=' + servicetype,
    Smoothbox.open(url);
	}
	else { 
	  en4.core.showError("Please select at least one friend to invite.");
