...
// 			}
  ...
  function onInstall() {
    ...
    if (!empty($type_array)) {
      ...
      $length_type = $length[0];
      if ($length_type < 32) {
        $db->query("ALTER TABLE `engine4_authorization_allow` CHANGE `resource_type` `resource_type` VARCHAR( 32 ) CHARACTER SET latin1 COLLATE latin1_general_ci NOT NULL");
      }
    }
    ...
    if (!empty($type_array)) {
      ...
      $length_type = $length[0];
      if ($length_type < 32) {
        $db->query("ALTER TABLE `engine4_authorization_permissions` CHANGE `name` `name` VARCHAR( 32 ) CHARACTER SET latin1 COLLATE latin1_general_ci NOT NULL");
      }
    }
