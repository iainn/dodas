...
//    $currenturl = ( _ENGINE_SSL ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] . $_SERVER['HTTP_HOST'] . $front->getRequest()->getRequestUri();
         ...
         if (empty($google_Apikey))
				...
				if (!empty($result)) {
					...
					if (!empty($GoogleContacts)) { 
					  if ($_POST['moduletype'] == 'sitepage' || $_POST['moduletype'] == 'sitebusiness') { 
					    $this->view->moduletype = $_POST['moduletype'];
      ...
      header('location: ' . $FBloginURL);
		...
		if (!empty($_POST['task']) && $_POST['task'] == 'get_facebookcontacts') { 
		  ...
		  if ($_POST['moduletype'] == 'sitepage' || $_POST['moduletype'] == 'sitebusiness') {
			...
			$friends_invite = $facebookapi->facebook_invitefriend($callback, $_POST['invitepage_id'], $_POST['moduletype']);
			echo Zend_Json::encode(array('friends_invite' => $friends_invite['exclude_ids'], 'message' => $friends_invite['message'])); 
			exit();
		}
      exit();          
  }
  ...
  public function getlinkedincontactsAction () {
	...
	public function getwindowlivecontactsAction () {
	 ...
	 include_once APPLICATION_PATH . '/application/modules/Seaocore/Api/Windowlive/contacts_fn.php';
  		...
  		$wll = WindowsLiveLogin::initFromXml($KEYFILE, $callback);
  		...
  		$token = null;
  		 if ($consent) { 
  		  $token = $wll->processConsentToken($consent->getToken());
      ...
      //HERE WE ARE CHECKING IF THE VALID TOKEN IS ALREADY COOKIED AND ACTIVE AND REQUEST IS NOT AJAX THEN WE WILL REDIRECT IT TO CALLBACK URL.
  		...
  		if ($token && !isset($_POST['task'])) {  
  		  $callback = $callback . '?oauth_verifier=' . $consent->getToken();
		...
		if ( isset ($_POST['task']) && isset ($_POST['oauth_verifier'])) { 
			...
			$WindowLiveContacts = get_people_array($wll->processConsentToken($_POST['oauth_verifier']));
			$session->windowlivemsnredirect = 0;		
			//NOW WE WILL PARSE THE RESULT ACCORDING TO HOW MANY USERS ARE MEMBERS OF THIS SITE.
  ...
  public function gettwittercontactsAction () {
    ...
    //STEP:2 AFTER GETTING REQUESTED OAUTH TOKE AND OAUTH TOKEN SECRET WE WILL GET OAUTH VERIFIER BY GRANTING ACCESS TO THIRD PARTY FOR FATCHING YAHOO CONTACTS.
     ...
     $LinkedinContacts = $Api_twitter->retriveContacts ($_POST['moduletype']);
     $this->view->moduletype = $_POST['moduletype']; 
			//if($response['success'] === TRUE) { 
			   $session->twitterredirect =0;
  ...
  function invitetositeAction() { 
	   ...
	   //if (!empty($invite_friend)) {
    	  ...
    	  if ($_POST['socialtype'] == 'linkedin') { 
    	    $recepients = array ();
    	    foreach ($friendsToJoin as $userid => $friend) {
    	      
    	      $recepients[] = (string)$friend;
    	    }
    	    
    	    $Api_linkedin = Seaocore_Api_Linkedin_Api::sendInvite ($recepients, null, $_POST['invitepage_id'], $_POST['moduletype']);
          //$Api_linkedin->sendInvite ($friendsToJoin);
    	  }
    	  else if ($_POST['socialtype'] == 'twitter') { 
    	   
    	    $Api_linkedin = Seaocore_Api_Twitter_Api::sendInvite ($friendsToJoin, '', $_POST['invitepage_id'], $_POST['moduletype']);
          //$Api_linkedin->sendInvite ($friendsToJoin);
    	  }
    	  else { 
    	      if ($_POST['moduletype'] == 'sitepage' || $_POST['moduletype'] == 'sitebusiness') {
    	        Engine_Api::_()->getApi('Invite', 'Seaocore')->sendPageInvites($friendsToJoin, $_POST['invitepage_id'], $_POST['moduletype']);
    	      }
    	      else { 
    	         Engine_Api::_()->getApi('Invite', 'Seaocore')->sendInvites($recepients, $_POST['socialtype'], $_POST['moduletype']);
    	      }
