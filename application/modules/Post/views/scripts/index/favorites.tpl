...
<h3 class="sep post_main_header">
  ...
  <?php if ($this->category && $this->categoryObject instanceof Post_Model_Category): ?>
    <?php echo $this->translate("My Favorited Posts: %s", $this->translate($this->categoryObject->getTitle())); ?>
  <?php else: ?>
    <?php echo $this->translate("My Favorited Posts"); ?>
  <?php endif; ?>    
  </span>
</h3>    
...
<?php if( $this->paginator->getTotalItemCount() > 0 ): ?>
  ...
  <div class="posts_result_pager">
    <?php echo $this->translate('Showing posts'); ?>
      <?php if ($this->tag): ?>
        <?php echo $this->translate('tagging #%s', $this->htmlLink(
          $this->url(array('action'=>'favorites', 'tag'=>$this->tag), 'post_general', true),
          $this->tagObject ? $this->tagObject->text : $this->tag
        ));?>
      <?php endif; ?>    
      <?php if ($this->keyword): ?>
        <?php echo $this->translate('with %s keyword', $this->htmlLink(
          $this->url(array('action'=>'favorites', 'keyword'=>$this->keyword), 'post_general', true),
          $this->keyword
        ));?>
      <?php endif; ?>    
      <?php if ($this->user): ?>
    ...
    <?php echo $this->htmlLink(array('action'=>'favorites', 'route'=>'post_general'), $this->translate('(x)'))?>
  </div>
  <?php endif; ?>
...
<?php else:?>
  <div class="tip">
    <span>
      <?php echo $this->translate('There are no posts voted yet.');?>
    </span>
  </div>
<?php endif; ?>
